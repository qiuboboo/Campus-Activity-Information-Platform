...
| 方法 | 路径 | 说明 | 权限 |
|------|------|------|------|
| GET | `/api/activities` | 列表，支持 `?q=&status=&page=&per_page=` | 登录 |
| POST | `/api/activities` | 创建，body `{"raw_text": "..."}` | publisher+ |
| GET | `/api/activities/{id}` | 详情 | 登录 |
| PUT | `/api/activities/{id}` | 更新 | publisher+ |
| POST | `/api/activities/{id}/submit` | 提交审核（draft→pending_review） | publisher+ |
| POST | `/api/activities/{id}/review` | 审核通过/驳回 | admin |
| GET | `/api/activities/review-queue` | 审核队列，支持 `?status=&sort_by=` | admin |
| POST | `/api/activities/bulk-review` | 批量审核 | admin |
| GET | `/api/activities/{id}/related` | 关联知识节点 | 登录 |
| GET | `/api/activities/{id}/duplicates` | 查看重复 | admin |
| POST | `/api/activities/{id}/merge-source` | 合并来源 | admin |
| POST | `/api/activities/{id}/rebuild-knowledge` | 重建知识节点 | admin |
| POST | `/api/activities/{id}/ai-enrich` | AI 增强提取 | admin |
...
| POST | `/api/ai/enrich/{activity_id}` | 增强活动提取标签/摘要 |
...
| GET | `/api/export/activities.json` | 导出全部活动 |
...
sequenceDiagram
    ...
    用户->>API: GET /api/activities (浏览活动)
    用户->>API: GET /api/activities/{id} (查看详情)
    用户->>API: GET /api/activities/{id}/related (查看关联知识)
...
```mermaid
sequenceDiagram
    发布者->>API: POST /api/activities (创建草稿)
    发布者->>API: POST /api/activities/{id}/submit (提交审核)
    管理员->>API: GET /api/activities/review-queue (查看队列)
    管理员->>API: POST /api/activities/{id}/review (审核通过)
```
...
| 文档 | 位置 | 适合谁 |
|------|------|--------|
| 后端技术文档（拆分版） | `docs/backend/README.md` | 全面了解架构和 API |
| 后端技术文档（原版） | `docs/后端技术文档.md` | 完整备份 |
