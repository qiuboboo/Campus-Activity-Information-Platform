  ...
  <div class="login-wrapper">
    ...
    <div class="half-transparent-card">
      <el-form
        ...
        class="login-form"
      >
        <h2 style="text-align: center; margin-bottom: 16px; color: #303133; font-size: 24px;">用户登录</h2>
        ...
        <el-form-item>
          <div class="form-options">
            ...
            <span style="font-size: 15px; color: rgba(26,26,46,0.45);">
              <router-link to="/auth/forgot-password" style="color: rgba(26,26,46,0.45); text-decoration: none;">忘记密码</router-link>
              <span style="margin: 0 6px; color: rgba(26,26,46,0.25);">|</span>
              <router-link to="/auth/register" style="color: #0b7d5b; font-weight: 500;">立即注册</router-link>
            </span>
          </div>
        </el-form-item>
...
<style scoped>
.login-wrapper { height: 100vh; width: 100vw; display: flex; justify-content: center; align-items: center; position: relative; overflow: hidden; }
.login-bg { position: absolute; inset: 0; z-index: 0; background: url('@/assets/huaishitang.jpg') center / cover no-repeat; }
.login-overlay { position: absolute; inset: 0; z-index: 1; background: linear-gradient(135deg, rgba(0,0,0,0.45) 0%, rgba(0,0,0,0.25) 50%, rgba(0,0,0,0.40) 100%); }
...
.login-form :deep(.el-form-item__label) { color: rgba(26,26,46,0.65); font-size: 13px; font-weight: 500; padding-bottom: 4px; }
.login-form :deep(.el-input__wrapper) { background: rgba(255,255,255,0.7); border: 1px solid rgba(26,26,46,0.10); border-radius: 10px; box-shadow: none; transition: all 0.25s ease; }
.login-form :deep(.el-input__wrapper:hover) { border-color: rgba(11,125,91,0.40); background: rgba(255,255,255,0.9); }
.login-form :deep(.el-input__wrapper.is-focus) { border-color: #0b7d5b; background: #fff; box-shadow: 0 0 0 3px rgba(11,125,91,0.15); }
.login-form :deep(.el-input__inner) { color: #1a1a2e; }
.login-form :deep(.el-input__inner::placeholder) { color: rgba(26,26,46,0.30); }
.login-form :deep(.el-input__prefix) { color: rgba(26,26,46,0.35); }
.login-form :deep(.pwd-toggle) { font-size: 22px; color: rgba(26,26,46,0.35); cursor: pointer; border-radius: 6px; padding: 4px; transition: all 0.2s ease; }
.login-form :deep(.pwd-toggle:hover) { color: #0b7d5b; background: rgba(11,125,91,0.08); }
.login-form :deep(.el-form-item.is-error .el-input__wrapper) { border-color: #f56c6c; box-shadow: 0 0 0 3px rgba(245,108,108,0.12); }
.form-options { display: flex; justify-content: space-between; align-items: center; width: 100%; }
.form-options :deep(.el-checkbox__label) { font-size: 13px; color: rgba(26,26,46,0.55); }
.login-btn { width: 100%; height: 48px; font-size: 16px; font-weight: 600; letter-spacing: 2px; border-radius: 10px; background: linear-gradient(135deg,#0b7d5b,#1bbf7a); border: none; transition: all 0.3s ease; }
.login-btn:hover { background: linear-gradient(135deg,#0ea36f,#22c98a); transform: translateY(-1px); box-shadow: 0 8px 20px rgba(11,125,91,0.35); }
...
@media (max-width: 480px) { .back-home { top: 12px; left: 12px; } .top-center-title h1 { font-size: 28px; } .half-transparent-card { padding: 24px 18px 20px; } .login-btn { height: 44px; font-size: 15px; } }
</style>
