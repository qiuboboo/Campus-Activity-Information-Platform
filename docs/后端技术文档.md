...
- **登录缺少图形验证码**：目前登录接口仅依赖速率限制（10 次/分钟/IP）防暴力破解，缺少图形验证码机制。后续需增加验证码生成与校验流程，降低自动化工具暴力破解和 DDoS 风险。详见 [GitHub Issue #1](https://github.com/qiuboboo/Campus-Activity-Information-Platform/issues/1)。
...
- 登录接口限制为 10 次/分钟/IP（待补充图形验证码），防止暴力破解
