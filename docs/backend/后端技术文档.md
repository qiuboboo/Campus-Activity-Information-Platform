...
> ⚠️ 本文档已按模块拆分为独立文件，详见 [docs/backend/README.md](backend/README.md)。
> 此文件保留为备份（原内容未删除），后续更新请修改 `docs/backend/` 下对应文件。
...
- 个人活动日历
  - 用户可将感兴趣的活动一键导入个人日历（生成 .ics 文件）
  - 支持"我的日历"视图，按时间线展示已收藏的活动
- 个性化订阅与通知
  - 用户可订阅知识节点（地点/主办方/活动类型），新活动发布时自动匹配并推送通知
...
- **已实现**：图形验证码已集成到登录和注册接口（`GET /api/auth/captcha` 获取验证码，提交时携带 `captcha_token` + `captcha_code`）。详见 [GitHub Issue #1](https://github.com/qiuboboo/Campus-Activity-Information-Platform/issues/1)。
- **已实现**：邮箱验证码注册流程（`POST /api/auth/send-code` 发送 6 位验证码到邮箱，注册时需携带 `email` + `verification_code` + `captcha`）。详见 [GitHub Issue #2](https://github.com/qiuboboo/Campus-Activity-Information-Platform/issues/2)。
...
根据目标公众号（如"逸仙look"）的活动抓取需求，系统调研了两种公众号文章获取方案。

**方案一：搜狗微信搜索（已实现）**

- 通过 `weixin.sogou.com` 搜索关键词，获取文章标题、发布时间和搜狗跳转链接
- 支持多页搜索 + 按时间排序获取最新文章
- 支持搜狗高级语法（`""`精确匹配、`-`排除噪音词）
- 支持搜狗反爬检测（`_is_antispider()`）

**已知限制：**
- `resolve_real_url()` 曾因搜狗反爬拦截而无法获取文章真实 URL。解决方案：通过 `SOGOU_COOKIES` 环境变量注入浏览器 Cookie（SNUID/SUID/SUV 等），即可绕过反爬，成功解析到 `mp.weixin.qq.com` 真实 URL
- `mp.weixin.qq.com` 正文页面本身也有微信风控（"环境异常"），纯 HTTP 请求仍无法拿到正文。如需全文抓取，需额外使用 Playwright 或 Cloudflare Worker 代理
...
参考 [wechat-article-exporter](https://github.com/wechat-article/wechat-article-exporter) 的实现，通过微信公众平台后台 API 获取文章：

```
扫码登录 mp.weixin.qq.com → 拿到 Cookie + token(4天有效)
    → 搜索目标公众号 → 拿到 fakeid
    → 通过 fakeid 获取文章列表 → 拿到 mp.weixin.qq.com 真实URL
    → Cloudflare Worker 代理 → 抓正文
```

**该方案的优势：**
- 直接获取 `mp.weixin.qq.com` 真实文章 URL，无需经过搜狗跳转
- 可限定到指定公众号，结果精准
- 可拿到文章全文

**该方案的风险与门槛：**
- 需要有已注册的微信公众号（个人订阅号免费即可）
- 每 4 天需要手动扫码登录续期，无法完全无人值守
- 微信接口有频率限制（`200013 freq control`，约 600 条/20 分钟触发，封禁 1-2 小时）
- 自动化操作违反微信服务条款，存在账号功能限制风险

**决策：**

对于低频率、针对特定公众号（如逸仙look）的活动抓取场景，方案二的可行性和风险在可接受范围内。后续如需接入，可按照以下步骤实施：

1. 注册个人微信公众号
2. 实现 MP 后台扫码登录流程，Cookie 持久化
3. 实现公众号搜索与文章列表拉取
4. 部署 Cloudflare Worker 代理节点用于正文抓取
5. 将抓取结果接入现有 Poster 草稿流程

当前阶段暂以方案一（搜狗搜索 + 标题时间提取）为主，满足活动线索发现需求。待确认公众号注册和扫码续期的运维安排后再切换到方案二。
...
3. 第三优先级：外部搜索引擎补充，搜索非结构化校园活动线索。当前已通过 SearXNG 多引擎搜索（Google、Bing、DuckDuckGo、百度）+ 搜狗微信搜索实现真实搜索结果聚合。
...
- 外部搜索
  ...
  - 非 AI 实现：SearXNG 多引擎搜索（Google、Bing、DuckDuckGo、百度）+ 搜狗微信搜索
- 网页内容解析
...
功能：注册新用户。默认角色为 `viewer`，可指定 `publisher`。不允许自注册为 `admin`。需携带邮箱验证码。

速率限制：5 次/分钟/IP，防止滥用注册。
...
错误码：
- `400`：参数校验失败（用户名长度、密码长度、角色不合法、邮箱不合法、验证码错误/过期）
- `409`：用户名或邮箱已存在

### `GET /api/auth/captcha`

功能：获取图形验证码。返回 PNG 图片，验证码令牌（`X-Captcha-Token`）在响应头中。
...
2. 用户输入验证码数字
3. 登录/注册时提交 `captcha_token`（来自 `X-Captcha-Token` 头）和 `captcha_code`（用户输入的数字）

### `POST /api/auth/send-code`

功能：发送邮箱验证码。接收邮箱地址，生成 6 位数字验证码存入 Redis（5 分钟有效，60 秒冷却），通过 SMTP 发送邮件。
...
功能：用户登录并获取 JWT。需要携带验证码。支持用户名或邮箱登录。

速率限制：10 次/分钟/IP（配合验证码防暴力破解）。
...
功能：创建订阅，支持按知识节点或关键词订阅。body: `{"node_id": 7}` 或 `{"keyword": "讲座"}`。
...
功能：下载单张海报的 .ics 日历文件（无需认证，公开）。

#### `POST /api/calendar/events`

功能：将海报添加到"我的日历"。body: `{"poster_id": 14}`。
...
详细的接口契约（字段类型、`hit_type` 枚举、null 语义、完整 JSON 示例）见 [search-contract.md](search-contract.md)。

说明：语义向量检索仅当 `EMBEDDING_ENABLED=true` 且已配置 pgvector 时启用。

### `GET /api/search/external?q=...`

功能：通过多引擎搜索（Google、Bing、DuckDuckGo、百度、搜狗）获取外部活动信息。支持 LLM 兜底。

默认启用 5 个搜索引擎，结果按引擎来源标记（`results[].source`）。可通过 `sources` 参数指定搜索源。

详细的接口契约（字段类型、error 取值、完整 JSON 示例）见 [search-contract.md](search-contract.md)。
...
功能：获取首页推荐活动列表（公开，无需认证）。返回最近发布的 3 条已上线海报，包含标题、摘要、时间、地点、主办方、活动类型。
...
功能：健康检查（公开，无需认证）。返回数据库与 Redis 连接状态。
...
| `MAIL_PASSWORD` | SMTP 登录密码（QQ邮箱为授权码） |
| `MAIL_DEFAULT_SENDER` | 发件人地址 |
| `CORS_ORIGINS` | 允许的跨域来源（默认 `*`） |
| `SOGOU_COOKIES` | 搜狗微信搜索 Cookie（SNUID/SUID 等，用于绕过反爬） |
